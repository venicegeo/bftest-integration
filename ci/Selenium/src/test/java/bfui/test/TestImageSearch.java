...
public class TestImageSearch {
	...
	public void exercise_cloud_slider() throws Exception {
		...
		assertTrue("Cloud cover slider should move to center",
				createJobWindow.cloudSliderValue() > 45 && createJobWindow.cloudSliderValue() < 55);
		...
		assertTrue("Cloud cover display should match value",
				createJobWindow.cloudText.getText().contains(cloudSlider.getAttribute("value")));
	...
	public void bad_planet_key() throws Exception {
		...
		// assertTrue("Error message should say that the problem is with credentials",
		// createJobWindow.errorMessageDescription.getText().matches("(?i).*CREDENTIALS.*"));
	}
